...
public class Robot_Mat_Ego {
    public static void moveRobot(RobotConnectionManager robotConnectionManager, int toX, int toY) {
        RobotConnection rob = null;
        for (int i = 0; i < 3; i++) {
            try {
                rob = robotConnectionManager.getConnection();
                rob.moveRobotTo(toX, toY);
                return;

            } catch (RobotConnectionException e) {

                if (i == 2) {
                    throw new RobotConnectionException("End");
                }
            } finally {
                try {
                    rob.close();
